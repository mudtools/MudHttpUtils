...
public static class VerifyFixture
{
        ...
        [CallerFilePath] string sourceFile = "")
    {
        ...
        var generatedTrees = outputCompilation.SyntaxTrees.Skip(1).ToHashSet();

        // [CS0219 形状守卫] 生成代码不得出现「已赋值但从未使用」的局部变量。
        // 历史缺陷：方法级无条件宣告 `__bodyContent` 槽位，而只有 FormUrlEncoded 体分支会写它，
        // 其余方法全部产出 CS0219 —— 该告警随生成代码进入消费方编译输出，消费方启用
        // TreatWarningsAsErrors 即构建失败，且库方无法替消费方关闭。
        // 注意：`// <auto-generated/>` 头**不能**抑制 CS0219（编译器流分析告警，非 IDE/样式分析器），
        // 故只能靠「生成形状」约束，不能靠抑制。
        var cs0219 = outputCompilation.GetDiagnostics()
            .Where(d => d.Id == "CS0219" && d.Location.SourceTree != null && generatedTrees.Contains(d.Location.SourceTree))
            .ToArray();
        cs0219.Should().BeEmpty(
            "生成代码不得含 CS0219（赋值未使用的局部变量），否则该告警会泄漏到消费方构建；命中：" +
            string.Join("\n", cs0219.Select(d => d.ToString())));
