...
public class MtRoundRegressionTests
{
    ...
    public async Task ClientSecretCache_ZeroTtl_ShouldNotCache()
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            var value = await cache.GetAsync(_ =>
            {
                calls++;
    ...
    public async Task ClientSecretCache_EmptyResult_ShouldNotBeCached()
    {
        ...
        await cache.GetAsync(_ => { calls++; return Task.FromResult<string?>(null); }, CancellationToken.None);
        await cache.GetAsync(_ => { calls++; return Task.FromResult<string?>("real"); }, CancellationToken.None);
    ...
    public async Task ClientSecretCache_TtlHotReload_ShouldTakeEffectWithoutRecreation()
    {
        ...
        await cache.GetAsync(_ => { calls++; return Task.FromResult<string?>("s"); }, CancellationToken.None);
        await cache.GetAsync(_ => { calls++; return Task.FromResult<string?>("s"); }, CancellationToken.None);
        calls.Should().Be(1, "TTL 窗口内应命中缓存");
