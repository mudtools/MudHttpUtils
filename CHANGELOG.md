...
#### 补充修复（FIX-02 复核 —— 请求体内容所有权收敛）

- **生成代码的 CS0219 泄漏到消费方构建**：`MethodGenerator` 无条件发射方法级槽位
  `System.Net.Http.HttpContent? __bodyContent = null;` 供「条件创建请求体内容」使用，
  而全库只有 `[Body] + [SerializationMethod(FormUrlEncoded)]` 分支会写该槽位 ⇒
  **其余每个生成方法都带一个「已赋值但从未使用」的局部变量**，生成代码编译即 CS0219。
  该告警随生成代码进入消费方编译输出：消费方启用 `TreatWarningsAsErrors` 即构建失败，
  而库方无法替其关闭（`// <auto-generated/>` 头**不**抑制编译器流分析告警，实测确认）。
  现改由**创建点**承担所有权（与既有 JSON/XML/表单路径同一原则）：
  ① 条件创建（FormUrlEncoded 体）→ 创建点直接 `__httpRequest.Content = new FormUrlEncodedContent(...)`
  （所有权转移，由方法级 `using var __httpRequest` 在**发送之后**释放，不再引入任何可释放局部变量）；
  ② 无条件创建 → 保持方法级 `using var`。同时修正该分支在「无需 null 检查」时产物多缩进一级的问题。
  **请求语义与释放时机不变**（内容仍在发送完成后释放），仅生成产物形状变化：每个方法少一行，
  FormUrlEncoded 体分支的赋值形态变化。
- **同族缺陷的双侧复发守卫**：快照侧（`VerifyFixture`）与编译断言侧（`GeneratorCompileAssert`）
  统一断言「生成产物不得出现 CS0219」——覆盖有/无快照基线的路径；
  另补 FormUrlEncoded 体分支的单元测试（所有权形状、缩进自适应）与端到端编译用例
  （该分支此前**无任何端到端覆盖**，这正是缺陷长期未被发现的原因）。
