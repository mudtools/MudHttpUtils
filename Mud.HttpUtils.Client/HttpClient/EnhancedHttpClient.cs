...
() => DownloadLargeFileAsync(request, filePath, bufferSize: bufferSize, overwrite: overwrite, progress: progress, cancellationToken: cancellationToken));
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // H-2 修复（设计权衡说明）：此处将所有非 HttpRequestException / OperationCanceledException 的异常统一包装为 HttpRequestException，
            // 会丢失原始异常类型（如 JsonException、InvalidOperationException），调用方无法按具体类型 catch。
            // 这是有意为之的统一出口，原始异常通过 InnerException（构造参数 ex）完整保留，可经 InnerException 检查具体类型。
            // 如需保留原始异常类型语义，可在派生类重写此发送流程或在更上层直接处理底层异常。
    ...
    /// <remarks>
    /// M-11 修复提示：此方法在基类中直接创建新的 <c>HttpClient</c> 实例，绕过 <c>IHttpClientFactory</c> 池化机制。
    /// 频繁调用可能导致 Socket 端口耗尽和 DNS 不刷新问题。
    /// <para>
    /// 使用 <see cref="HttpClientFactoryEnhancedClient"/> 时，此方法已被重写为通过 <c>IHttpClientFactory.CreateClient</c> 创建新实例，
    /// 不会绕过池化机制。仅在直接继承 <see cref="EnhancedHttpClient"/> 且未重写此方法时才会触发上述问题。
    /// </para>
    /// </remarks>
    public virtual IEnhancedHttpClient WithBaseAddress(Uri baseAddress)
    {
        ...
        // M-11 / H-1 修复：在基类中发出警告日志，提示此路径绕过了 IHttpClientFactory，频繁调用可能导致 Socket 端口耗尽。
        // 自定义子类应重写 WithBaseAddress 以使用 IHttpClientFactory 进行套接字池化管理。
        if (_enableLogging && _logger.IsEnabled(LogLevel.Warning))
        {
            _logger.LogWarning(
                "EnhancedHttpClient.WithBaseAddress 创建了不受 IHttpClientFactory 管理的 HttpClient 实例。" +
                "频繁调用可能导致 Socket 端口耗尽。建议使用 HttpClientFactoryEnhancedClient 重写此方法。");
        }
