...
> `IApiKeyProvider` 用于 API Key 认证模式（`TokenInjectionMode.ApiKey`），默认实现从 `IConfiguration` 读取密钥。`IHmacSignatureProvider` 用于 HMAC 签名认证模式（`TokenInjectionMode.HmacSignature`），默认实现使用 HMAC-SHA256 算法。
