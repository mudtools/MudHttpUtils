...
| `ICacheResponseInterceptor`| 缓存响应拦截器，配合 `CacheAttribute` 实现响应缓存 |
| `IHttpResponseCache`       | 响应缓存契约，提供 `TryGet`、`Set`、`Remove` 方法 |
| `IResiliencePolicyResolver`| 弹性策略解析器，支持自定义策略实现                |
| `IAppResiliencePolicyResolver`| 按应用（App）解析弹性策略的工厂接口（`ResolveResolver(string)`）|

> `IHttpResponseCache` 是缓存抽象，默认实现为 `MemoryHttpResponseCache`（基于 `IMemoryCache`）。可替换为 Redis 等分布式缓存实现。`ICacheResponseInterceptor` 继承 `IHttpResponseInterceptor`，在响应阶段检查缓存并存储响应。`IResiliencePolicyResolver` 用于解析弹性策略，默认实现为 `PollyResiliencePolicyProvider`（位于 `Mud.HttpUtils.Resilience` 包）。

### HTTP 请求执行器接口

| 接口                  | 说明                                                                                     |
| --------------------- | ---------------------------------------------------------------------------------------- |
| `IHttpRequestExecutor`| HTTP 请求执行器，统一处理响应反序列化、错误处理和执行模式编排。由源生成器生成的代码调用。|
| `ExecutionDescriptor` | 执行描述符，封装请求执行所需的元数据（拦截器、缓存、Token 恢复等）                       |
| `ResponseDescriptor`  | 响应描述符，封装响应处理方式（反序列化类型、错误处理、允许任意状态码等）                 |

> `IHttpRequestExecutor` 是生成的 API 实现类与运行时之间的桥梁，统一处理 HTTP 响应反序列化、错误状态码异常抛出、拦截器调用等逻辑。

### 可观测性接口与静态源

| 类型                        | 说明                                                                   |
| --------------------------- | ---------------------------------------------------------------------- |
| `MudHttpActivitySource`     | 分布式追踪源（ActivitySource），W3C TraceContext 兼容，无监听器时零开销|
| `MudHttpMeter`              | 指标源（Meter），提供 HTTP 请求计数、耗时、缓存命中、令牌刷新等指标    |
| `MudHttpDiagnosticListener`| 诊断监听器，将 HTTP 事件转发到 `DiagnosticListener`                    |
| `MudHttpDiagnosticNames`   | 诊断事件名称常量                                                       |
| `TokenRefreshStatsCollector`| 令牌刷新统计收集器，记录刷新次数和耗时                                |
| `CircuitBreakerStateObserver`| 熔断器状态观察器（`SetState`/`RemoveState`/`CurrentStates`），配合熔断可观测性 |
| `CircuitBreakerState`        | 熔断器状态枚举（`Closed` / `HalfOpen` / `Open`）                      |
| 诊断负载类型（`*DiagnosticPayload`）| `Mud.HttpUtils.Observability` 命名空间下：`CircuitBreakerDiagnosticPayload`、`TokenRefreshDiagnosticPayload`、`CacheDiagnosticPayload`、`DownloadDiagnosticPayload`、`DownloadErrorDiagnosticPayload` |

> 可观测性静态源（含上述类型）位于 **`Mud.HttpUtils.Observability`** 命名空间（非根命名空间），放在 Abstractions 项目中以便 `TokenManagerBase`、`EnhancedHttpClient`、`PollyResiliencePolicyProvider` 共用。配合 `Mud.HttpUtils.OpenTelemetry` 包可一键开启 Tracing + Metrics 导出。
...
>     "Key": "base64-encoded-16/24/32-byte-key"
>   }
> }
> ```
>
> **注意**：`Key` 必须为 Base64 编码的 16/24/32 字节密钥。注册时会在首次解析 `IEncryptionProvider` 时校验密钥长度。
...
| `ConcurrentDictionaryTokenCache<T>` | `ITokenCache<T>` 默认实现，基于 `ConcurrentDictionary`                                          |
| `TokenResult`                    | 只读令牌结果结构（AccessToken、ExpireTime、Scope、IsExpiringSoon 等）                               |
| `CurrentUserInfo`                | 当前用户基本信息基类（`UserId`），`UserTokenInfo` 继承自此类型                                      |
...
ExecutionDescriptor — 执行描述符（拦截器、缓存、Token 恢复）
ResponseDescriptor — 响应描述符（反序列化类型、错误处理）
...
- **可扩展性**：通过 `IEncryptionProvider`、`ITokenStore`、`IHttpClientResolver`、`IEnhancedHttpClientFactory`、`IApiKeyProvider`、`IHmacSignatureProvider`、`ISensitiveDataMasker`、`IHttpResponseCache`、`IResiliencePolicyResolver` 等接口支持自定义实现
