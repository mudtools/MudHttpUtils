...
> `IEnhancedHttpClient` 是 `IBaseHttpClient`、`IJsonHttpClient`、`IXmlHttpClient` 的组合接口，提供完整的 HTTP 客户端能力（JSON/XML 请求和下载）。加密功能由独立的 `IEncryptableHttpClient` 接口提供，需要加密能力的客户端应同时实现该接口。
...
```csharp
public class CustomHttpClient : IEnhancedHttpClient, IEncryptableHttpClient
{
...
├── IJsonHttpClient
├── IXmlHttpClient
└── IEnhancedHttpClient (继承 IBaseHttpClient, IJsonHttpClient, IXmlHttpClient)

IEncryptableHttpClient (独立接口，IEnhancedHttpClient 不继承)
