...
| 接口 / 类                        | 说明                                                                                                 |
| -------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `ITokenManager`                  | 通用令牌管理，提供 `GetTokenAsync`、`GetOrRefreshTokenAsync` 方法（**推荐 DI 生命周期：Singleton**） |
...
> `TokenRefreshBackgroundOptions` 控制后台刷新服务的行为，`UserTokenCacheOptions`（位于 `Mud.HttpUtils.Client` 包）控制用户令牌缓存容量和清理策略。
