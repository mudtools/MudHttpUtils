...
internal class AccessTokenGenerator : ICodeFragmentGenerator
{
    ...
    private void GenerateGetTokenAsyncMethod(StringBuilder codeBuilder)
    {
        ...
        codeBuilder.AppendLine("            {");
        codeBuilder.AppendLine("                // 延迟初始化：首次调用时设置默认应用上下文");
        codeBuilder.AppendLine("                appContext = _appManager.GetDefaultApp();");
        codeBuilder.AppendLine("                _appContextHolder.Current = appContext;");
        codeBuilder.AppendLine("            }");
